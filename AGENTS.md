...
- `BENCH_N` sets the benchmark record count (default 500 000; the CI bench workflow uses 50 000). `BENCH_PASSES` sets the minimum number of timed rounds per candidate in `bench-v4` (default 3, after at least 2 warmup rounds); both counts are rounded up to a multiple of the scenario's candidate count so the rotated order forms complete rotations (4 plus 4 with four candidates).
