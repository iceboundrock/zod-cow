...
1. **Official products as leaves/subtrees** (`official.ts`). Imports `compileFn`, `INVALID`, `ZodCompileUnsupportedError`, `ZodCompileAsyncError`, `$ZodAsyncError` from `zod4/v4/core` (a public subpath; the two `ZodCompile*` errors are public, the rest are unsupported compiler internals). `compileFn(schema)` gives a stock-semantics parser; `compileFn(schema, {assertOnly:true})` gives a validator that skips output construction.
