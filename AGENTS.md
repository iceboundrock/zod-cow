...
| `options.ts` | `CompileOptions` (the public options of `compile(schema, options?)`: `ownSymbolKeys: "probe" \| "ignore"`), the resolved `CowOptions`, `DEFAULT_OPTIONS`, `resolveOptions` (throws `TypeError` on an unknown value, `null` included, or a non-plain options object, and formats that error from property descriptors so a throwing accessor or Proxy trap on the rejected object cannot replace the `TypeError`; reads only an own `ownSymbolKeys`, never an inherited one, and treats an own `undefined` as absent) |
