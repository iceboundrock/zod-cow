...
- `bench-v4` measures ArkType as a first-class column in every scenario (S1 to S7) next to stock zod4, the official `compileFn` parser and `zod-cow-v4`, instead of one weakened reference line at the end. The ArkType schemas carry the same constraints as the zod schemas (`number.integer & number.safe` for `.int()`, `string <= 64`, `string[] <= 8`, the literal union with `= 'viewer'` for the default; the zod email and datetime patterns go in as ArkType regex constraints because ArkType's own `string.email` / `string.date.iso` keywords accept supersets). An equivalence gate (`packages/bench-v4/gates.ts`) runs valid and deliberately invalid fixtures through every implementation before anything is timed and aborts on an undeclared disagreement; declared divergences (a present-undefined key is defaulted by zod but rejected by ArkType, a present-undefined optional tuple slot likewise) are printed as `known divergence`. Scenarios ArkType 2.2.3 cannot express natively print `N/A — <reason>`: S5 (`Map` / `Set` are instanceof-only, no `Map<K, V>` / `Set<T>` generic; the closest schema is measured as a non-equivalent reference and kept out of the ratios) and S7 (a `.pipe(async fn)` morph returns an un-awaited Promise). Every timed call verifies its own result, timed rounds are interleaved with a rotated candidate order (`packages/bench-v4/harness.ts`, `BENCH_PASSES` sets the round count), and the run ends with three markdown summary tables: median time, allocation / retained, and ratios against `zod-cow-v4` (values above 1 mean zod-cow was faster).
