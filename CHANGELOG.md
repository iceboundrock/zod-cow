...
- `docs/adr/0001-package-layout.md` records the package layout decision for the upcoming split (#8): a pnpm workspace with one package per zod major, `zod-cow` published for the zod4 line with a peer range on zod limited to verified minors (initially `>=4.5.4 <4.6.0`), the zod3 line kept private, ESM-only output. #9 implements it.
