...
- Symbol keys / getters: stock's rebuild drops undeclared own symbol keys (enumerable or not) in every object mode, and by default the object skeleton probes for them in every mode and copies (strip since #33, strict and loose since #42). Under `compile(schema, { ownSymbolKeys: "ignore" })` the probe is skipped: a clean input keeps its own symbol keys by reference, while a copy made by the skeleton still drops them like stock, so the outcome then depends on whether the object was dirty (#43). The zod4 object skeleton reads a getter once on both paths, like stock; the array, record, map and set skeletons copy with `slice()` / `{ ...input }` / `new Map(input)` / `new Set(input)` on the dirty path, which reads an accessor property a second time (#36).
