...
- A stock quirk that is deliberately not matched: with an async rest element and a `null` input in a nullable slot, stock zod4's runtime produces a sparse array and loses the `null`; the skeleton outputs a dense array. The differential generator avoids that combination; the reproduction is in the upstream issue draft. A second quirk of the same kind: stock's runtime writes a sync record pair, map entry or set member as soon as it is parsed and an async one when its promise settles, after the loop, so an async parse hands back a Map or Set in settlement order and, on a key collision, lets an earlier async pair win over a later sync one (`z.set(z.union([z.string().refine(async …), z.number()]))` on `Set {"a", 1}` gives `Set {1, "a"}`). The skeletons keep iteration order on every path, as zod's own compiler does (it awaits each entry in place); the differential compares async outputs unordered and keeps its colliding-key variants sync (#67).
