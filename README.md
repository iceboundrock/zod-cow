...
- The clean path returns the input as it is, so what stock's rebuild normalizes away survives when nothing forced a copy: a non-enumerable undeclared string key (the `for...in` probe and the record key loop skip it), the input's prototype (a class instance comes back as that instance where stock returns a plain object; inherited enumerable keys are seen and take the copy path) and Proxy traps (a strip object enumerates on the clean path where stock's strip does not, a loose object does not enumerate where stock does, so a throwing `ownKeys` trap is observed differently). No probe is added for them: proving their absence on every clean container is the cost `ownSymbolKeys: "ignore"` exists to remove, and stock `parse` is there for inputs that need the normalization (#48).
