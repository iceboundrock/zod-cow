...
- Symbol keys / getters: a strict or loose object returned by reference keeps own symbol keys (enumerable or not) that stock's rebuild drops (strip mode probes for them and copies); a copy made by the skeleton drops them like stock, so the outcome depends on whether the object was dirty (#42). `compile(schema, { ownSymbolKeys: "ignore" })` opts strip-mode objects into the same behavior: the probe is skipped and a clean input keeps its own symbol keys by reference (#43). The zod4 object skeleton reads a getter once on both paths, like stock; the array, record, map and set skeletons copy with `slice()` / `{ ...input }` / `new Map(input)` / `new Set(input)` on the dirty path, which reads an accessor property a second time (#36).
