...
pnpm run test:v3     # zod3 线：65 个单元测试 + 20000 case 差分模糊（对比 stock zod3，含有序 issue 列表），生成骨架与闭包骨架各跑一遍
...
4. Async：async 子树变成 async 岛，所有产物调用位只在结果确实是 `Promise` 时才挂起，骨架变成按 stock runtime 的方式驱动的生成器。因此没有任何产物返回 `Promise` 的那次 parse 会像 stock 一样同步完成，数据从未进入其 async 部分的容器也就保住了自己在兄弟之间的位置（#105）。async 产物上的同步 API 抛 `$ZodAsyncError`，与 stock 一致。
...
- zod3 线有 65 个单元测试和自己的 20 000 case 差分模糊（`packages/zod-cow-v3/tests/differential.test.ts`）：随机的 object / array / tuple / record / map / set / union / discriminatedUnion schema、接受容器的透传叶子 `unknown`、union 分支可以是叶子、容器或带包装的 schema（于是会出现 `readonly` 之下的 union，以及 union 分支上的 `readonly` / `catch` / `transform`），配 optional / nullable / default / catch / readonly / refine / transform 包装、创建参数 error map 以及 string 的 `length` / `includes` / `startsWith` 检查、稀疏数组与 tuple、与后面条目冲突的 record / map 键 transform 和 set 成员 transform、record 输入上自有的 `__proto__` 数据属性，以及在一部分生成的容器上，stock 的重建会归一化掉的描述符和 stock 解析时会做的读取（#66，`tests/decorations.ts`）：定义为不可枚举的已声明键、自有 symbol 键、已声明键或未声明键上的计数 getter、继承的可枚举键、present-undefined 值、对象上自有的 `__proto__`、记录日志的 Proxy、数组或 tuple 下标上的访问器（计数、抛出、改写另一个下标或移动长度）、自有的 `Symbol.iterator`、在该 case 内于原型上被替换的数组迭代器协议（`Array.prototype[Symbol.iterator]` 与 `%ArrayIteratorPrototype%.next` 换成包装函数，把每次 `iterator` 与 `next` 调用记录到迭代器所遍历的被装饰容器的日志上）。这样的输入能观察到的每一次读取都按容器记录，并在检查输出之前与 stock 的日志比较：tuple 精确比较（其捕获就是 stock 的展开），数组与 record 经由文档记载的拷贝路径前缀重读比较，数组另外去掉 stock 的迭代器读取（#116），对象比较已声明键的 `get` 读取，在拷贝路径上还比较未声明键的读取；被装饰容器的拷贝必须带有 stock 的键集合与描述符；`.pure` 为 true 时输出必须是输入原引用，除非输入带有会让 stock 的组装必然拷贝的键（文档记载的 strip 例外）。这些 case 对比 stock zod3 的成败一致性与抛出的错误、`deepStrictEqual` 输出（Map 与 Set 的内容按迭代顺序比较）、零输入改动（描述符级快照；当访问器的副作用按设计改写输入时，比较两个实例彼此的状态）、输入与输出的冻结状态，并在每个失败 case 上比较有序的 issue 列表（每条 issue 按 stock 的顺序，带上它携带的每个属性：code、path、message、`fatal`、检查参数、union 的嵌套错误）。它跑两遍：生成骨架与闭包回退（`--no-codegen`），并打印每种装饰在 case 中的占比，在默认规模下设有下限。成功 case 的顶层引用共享率在这个生成器下约 81.5%（加入 #66 的装饰输入之前为 82.4%，其访问器副作用与未声明键会强制拷贝；tuple 骨架的输出改为 stock 的新数组、数组的显式 `undefined` 成员失去原引用之前为 85%，#65；加入稀疏、冲突和 `__proto__` 用例之前为 87%，这些用例按构造必然触发拷贝；在更早、较小的生成器下为 92.1%）。
...
- 刻意不对齐的 stock quirk：tuple 带 async rest 槽且 nullable 槽输入为 `null` 时，stock zod4 runtime 产生稀疏数组并丢掉 `null`；骨架输出稠密数组。差分生成器规避该组合，复现见上游 issue 草稿。异步调度：stock 的 runtime 在循环内启动每个条目的解析，同步结果立即写入，异步结果等其 promise 落定后写入，所以异步解析返回的 record、Map 或 Set 按落定顺序排列，键冲突时更早的异步键值对会胜过更晚的同步键值对（`z.set(z.union([z.string().refine(async …), z.number()]))` 对 `Set {"a", 1}` 得到 `Set {1, "a"}`）。set、map 与遍历键的 record 骨架遵循同一调度（只有条目按迭代顺序落定且未变化时才共享输入），差分对异步输出也做有序比较（#70）。zod 自己的编译器没有异步模式（抛出 `ZodCompileAsyncError`），所以 runtime 是唯一的 stock 参照。对象、enum 键 record、数组与 tuple 骨架按 stock 的顺序在第一次挂起之前调用每个子节点，并用一次 `Promise.all` 结算 async 的那些（#71）：它们的输出按键或下标寻址，所以与 stock 一致，只有异步对象的键顺序（即上文的键顺序限制）不同，而数组与 tuple 的拷贝路径用的是那次挂起之前捕获的读取（#77）。静态判定为 async 的骨架只在结果确实是 `Promise` 时挂起，否则像 stock 的 runtime 一样同步完成（#105）：数据从未把它驱入 async 部分的容器（空 set、缺席的 optional 键、nullable 之下的 `null`、未命中的 union 分支）因此落在 stock 的那一轮，而不是晚一轮。
