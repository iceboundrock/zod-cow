...
- property 抽样（#99）保持四十分之一的频率：其中三分之一针对保留的未声明键 `propertyExtra`，必需与可选 string 等概率，其余保留声明键 `f0` 的 check。每种 object 模式都独立地以四分之一概率生成隐藏自有键、十分之一概率生成隐藏继承键，取字符串或非字符串值，与现有额外键并存。重放也展示容器内的这些属性。每个 case 用默认选项与 `ownSymbolKeys: "ignore"` 各跑一次，schema 与 RNG 流相同，输入只去掉额外 symbol。两遍的 schema/default 探测一致，检查每个 case 的 RNG 抽取次数；`"ignore"` 下任何深度的生成骨架都不得含 symbol 探测，且第二遍共享引用不得少于第一遍。在 Node 24.20.0、默认 200 × 100 规模下，**成功用例中的共享率为 82.0% / 83.1%**（全部用例中为 32.1% / 32.7%），stock 降级 0 次。**当前验证被 [#105](https://github.com/iceboundrock/zod-cow/issues/105) 阻塞：**默认规模与 50 000 case 运行都在每遍发现一处 Set 顺序不一致，`REPRO=150:57`；这是与 property check 无关的 async 调度缺陷，保留为失败用例而未排除。历史 v0.5 的 50 000 case 运行为成功 20 813 / 失败 29 187，成功用例共享率 89.1%，降级 0 次。
