...
单记录热循环（同一个小输入，每轮 1 000 000 次操作，构建详细错误的 S10 行为 100 000 次，取每次操作的中位纳秒数；用于和公开的单对象基准形状对照，不是产品工作负载）：
