...
- 回调看到的是 CoW 输出而不是新容器：容器之上的 `refine` / `superRefine` / `transform` 在其下没有任何改动时收到的是输入引用，而 stock 的回调收到的是 stock 刚重建的容器。仅因自身长度 / 大小检查而变脏的容器（`array.max`、`set.size`）也包括在内：值按原引用继续向上，issue 已记录。只有在回调内比较引用身份时才能观察到。在 `readonly` 或已触发的 `default` 之下，zod3 骨架运行于 stock 的重建模式，那里的回调与 stock 的回调一样收到新容器。
