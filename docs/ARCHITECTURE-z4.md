...
- `packages/zod-cow-v4/tests/smoke-z4.test.ts` (12 groups of behavioral assertions, the last one the `ownSymbolKeys` option: default and `"probe"` still copy on an undeclared symbol, `"ignore"` returns the input by reference with the symbol kept, keeps strip semantics for string keys and the copy path, validates declared symbol keys, reaches nested skeletons under every container (object, array, tuple, record, map, set), treats a non-enumerable undeclared symbol like an enumerable one, rejects an unknown value, an explicit `null` or a non-plain options object with `TypeError`, treats an explicit `undefined` as the default, ignores an `ownSymbolKeys` inherited from `Object.prototype`) + `packages/zod-cow-v4/tests/smoke-z4-containers.test.ts`
