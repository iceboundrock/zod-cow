...
since #85. The extended property and input draw of #99 measures 82.7% / 83.6% sharing among successful
cases at the default size, every case agreeing with stock; its acceptance criterion is the mutation test
of Appendix A, not its composition counters.
...
- Since #99 one object in five carries a `z.property` check, half of them on the reserved undeclared `propertyExtra` key and half on the declared `f0` key. An undeclared draw is aimed at one of the two configurations in which a wrong read of an undeclared key is observable at all, because `compile().safeParse` retries stock on `INVALID` and only "compiled accepts what stock rejects" reaches the comparison: a required check in strip or strict mode over a key stock's `for...in` does not yield and whose value is a string (stock's assembly leaves the key out, so reading the input instead of the prototype of a fresh object passes where stock fails), and an optional check in loose mode over an enumerated key whose value is not a string (stock's assembly appends it, so a skeleton that dropped loose's enumerated read sees `undefined` and passes). Each of the aim's four dimensions — mode, required / optional, enumerable / non-enumerable, string / non-string value — is perturbed one time in four, so every other combination still occurs, and the key is generated as an own and as an inherited property. Only registered generator-created prototypes are normalized to plain objects for enumerable-output equality; a prototype of the case input reaching an object of the output that is not an input object is a failure of its own, since the object skeleton's copy is an object literal and a prototype can only reach the output through the clean path. Mutation snapshots separately capture all own descriptors, hidden/symbol values, extensibility and generated prototype identity/state, recursively through containers, and the replay annotates hidden own and inherited data. Helper checks pin these distinctions, holes, class-instance boundaries and Map order; a startup guard pins the generator's 30 mode/check/key-form combinations and a floor of 20 objects in each observable configuration, reached in 224 of its 20 000-seed budget. Schema/default probing uses the same symbol policy in both passes; only case input generation omits the symbol under `"ignore"`, and per-case RNG draw counts must match.
- The acceptance criterion for that draw is a mutation test, not its composition counters. With the pre-#98 `input[key]` read of an undeclared key put back into `heldValue` in `emit-object.ts`, the default-size run fails 3 cases in the default pass and 5 in the `"ignore"` pass; with loose's enumerated read dropped so `heldValue` always reads `Object.prototype`, 7 and 7. Both mutants survived 20 000 and 50 000 cases of the first version of this generator, whose draw rates left about one object per pass in each observable configuration and never emitted the key enumerably at all, so the loose scan never ran (review of #106). The run prints the objects it generated per configuration (78 / 94 and 143 / 157 at the default size, out of 1 116 checked objects in 769 cases) and fails below a floor of 20.
- Current #99 verification on Node 24.20.0: at 200 × 100 cases, default / `"ignore"` success counts are 7 676 / 7 710, failure counts 12 324 / 12 290, sharing 82.7% / 83.6% among successes (31.8% / 32.2% overall), 0 degradations, every case agreeing with stock; at 500 × 100 cases 19 117 / 19 196 successes and 83.4% / 84.1%, with the same result. The rate moves up from the 82.5% / 83.3% of the generator before it: a non-enumerable undeclared key is invisible to strip's `for...in` probe and to strict's rejection, so no clean path is lost (#48), the enumerable form is emitted only on objects carrying the check, and the rest of the movement is the shifted stream. [#105](https://github.com/iceboundrock/zod-cow/issues/105) — a statically async empty Set that stock completes synchronously, while the generated async container/wrapper products add Promise hops and reverse two outer Set members — is unchanged and still open; the first version of this stream reached it at `REPRO=150:57`, the shifted stream no longer generates that case, and the standalone reduction in the issue remains its reproduction.
