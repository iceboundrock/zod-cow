...
| optional/nullable | the wrapper's own checks pass `leafChecksArePure` and the inner is pure | the value passes through; an overwrite, a superRefine or an async refine on the wrapper rewrites or times the value as it would on a leaf (#57), and above a container the validator would also keep the undeclared keys stock strips (#56) |
...
line fixes the leaf case of #57). The fuzzer never built this shape: a child took at most one wrapper, and its refine
...
- since #47 the generator emits unions (2 to 3 random options, one in four a discriminated union of two object branches), which the list above had claimed while no union was generated; on the unfixed engine the new generator failed 15 of 20 000 cases in the default pass and 11 in the `"ignore"` pass
- since #56 one wrapped child in three carries a second check on top of its wrapper (a sync or async refine, or an overwrite that upper-cases a string, #57) and the refine predicate rejects a container with exactly three entries as well as the string "forbidden"; on the unfixed engine that finds 5 of 20 000 cases in each pass, and the sharing rate at the default size is 85.4% (default) / 86.1% (`"ignore"`) among successful cases, the previous generator reading 85.1% / 86.0% on both engines, all output mismatches of the trap-four shape, and the fixed engine fails none. Top-level reference sharing among successful cases at the default size is 85.6% (default) and 86.2% (`"ignore"`), from 85.9% and 86.6% on the unfixed engine with the same generator (the union-of-containers CoW path given up by the rule) and 88.8% / 89.4% with the previous generator.
