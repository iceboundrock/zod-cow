...
| `emit-tuple-rest.ts` | §5.4 | `emitSyncRest`：同步 rest 布局的 rest 段与存在性判定（#87，#88），`nativeSliceFrom`、`restFromMethod`、`restFromIterator`；只被 `emit-tuple.ts` 导入，不在下面的循环之内 |
