...
调度和中止规则；fuzzer 的 property 抽样自 #85 起保留其 object 的骨架。#99 扩展 property 与输入抽样后，默认规模下成功用例共享率为 82.0% / 83.1%；运行仍保留与此无关的 async Set 顺序失败 #105（见 §8）。
...
- 自 #99 起，四十分之一的 object property 抽样中，三分之一针对保留的未声明键 `propertyExtra`，必需与可选 string schema 等概率；保留声明键 `f0` 的分支。每种 object 模式都独立生成不可枚举自有键（四分之一）和偶尔被遮蔽的不可枚举继承键（十分之一），取字符串或非字符串值，与现有额外键并存。可枚举输出比较只把已登记的生成器原型规范化为普通对象；改动快照另行递归记录全部自有描述符、隐藏/symbol 值、可扩展性、生成原型的身份与状态，容器内部亦然。重放标注隐藏自有与继承数据。辅助检查钉住这些区别、空洞、类实例边界与 Map 顺序。两遍的 schema/default 探测使用相同 symbol 策略，只在 `"ignore"` 的 case 输入生成阶段省略 symbol，并要求每个 case 的 RNG 抽取次数一致。
- #99 当前在 Node 24.20.0 上的验证：200 × 100 case 下，默认 / `"ignore"` 成功数为 7 814 / 7 864，失败数为 12 186 / 12 136，成功用例共享率 82.0% / 83.1%（总体 32.1% / 32.7%），降级 0 次。两遍均保留 `REPRO=150:57` 的一处输出顺序不一致；500 × 100 case 下也是每遍唯一的不一致（成功用例共享率 82.4% / 83.3%）。[#105](https://github.com/iceboundrock/zod-cow/issues/105) 记录简化复现：静态判为 async 的空 Set 在 stock 中同步完成，而生成的 async 容器/包装产物增加 Promise 跳数，颠倒外层 Set 的两个成员。该用例不含 property check 或隐藏键，没有通过生成器排除或放松顺序比较来隐藏它。默认规模的 property schema/输入计数覆盖 35 个必需与 48 个可选 case，含 42 个非 object 根、26 个含 union 的 schema、57 个 async 产物、33 个带隐藏自有键的输入和 13 个带生成原型的输入。这些可重叠计数描述 schema/输入组合，不证明每个 check 都会在先前校验失败后执行。
