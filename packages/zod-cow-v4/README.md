...
compile(User, { ownSymbolKeys: "ignore" }); // opt-in for data known to carry no symbol keys (JSON input): skips the own-symbol probe of every object and record, see CompileOptions
```
...
|---|---|---|
| `ownSymbolKeys` | `"probe"` (default), `"ignore"` | What an object skeleton, in every object mode (strip, strict, loose), and a record skeleton do about own symbol keys of the input. Stock zod rebuilds every object and record and so drops undeclared own symbol keys on every path (strict mode does not reject them: its unknown-key check sees string keys only; a record with a string key schema rejects an enumerable symbol key and drops a non-enumerable one); to return the input by reference, the skeleton first has to prove there are none. For an object or an enum-keyed record that is one `Object.getOwnPropertySymbols` call, about 31 ns on a 6-field record in the cited Benchmarks run and the largest fixed cost of the clean path; a record that iterates its keys (`z.record(z.string(), …)` and other non-enum key schemas) already lists them and marks a non-enumerable symbol dirty inside that loop at no extra cost. `"ignore"` skips that probe and that branch at every depth of the compiled tree and in every mode: an input whose declared keys are unchanged and which carries no undeclared string key that its mode would strip or reject is returned by reference even if it carries an own symbol key, which then survives where stock would drop it. For data that cannot carry symbol keys (JSON input, structured-clone output, records built from literals) the two modes give the same results. Declared symbol keys are unaffected and the copy path still drops undeclared symbols. A value other than the two strings throws a `TypeError` from `compile` (`null` included; an explicit `undefined` counts as the property being absent), as does an options argument that is not a plain object (an array, `null`, a class instance or any other object whose prototype is neither `Object.prototype` nor `null`); both rejections stay a `TypeError` even when the rejected object or value fights the diagnostic (a throwing `constructor` or `name` accessor on its prototype, a Proxy whose `getPrototypeOf` trap throws, a `toJSON` method or Proxy `get` trap on the rejected value, a bigint or a cyclic object), because the message is built from property descriptors and type tags and never runs the rejected thing's own code. Reading the option off the options object does run that object's own code (an accessor, a Proxy trap); a throw there is reported as a `TypeError` whose `cause` is the caller's error. Only an own `ownSymbolKeys` property is read, so the option cannot be inherited from a prototype: `compile(schema, {})` is `compile(schema)` even if `Object.prototype` has been given an `ownSymbolKeys` property. |
...
| `parseAsync(data)` | `(data: unknown) => Promise<z.output<T>>` | Async `parse`; works for sync and async schemas. A plain function that returns a `Promise` is not an async function to zod's compiler or to this layer, so such a schema compiles as sync (`async` is `false`) and the fast path meets the `Promise` at runtime; that parse then runs on stock's async runtime, whose output is stock's copy, and the callbacks called before the `Promise` run a second time there, the same duplicate as on a failing parse. A `$ZodAsyncError` a callback throws itself is the caller's: thrown from a container, wrapper or union `.refine`, or by an awaited async callback, it rejects the parse after one call, as stock does; thrown from a leaf `.refine` / `.check` / `.superRefine` / `z.custom` predicate, a custom string format's predicate, `overwrite` or `transform` inside an official product it is recorded too (this layer wraps every non-async callback for the duration of the compile), so it also rejects after one call, the one residual being a `.default()` value factory (#80). |
| `safeParseAsync(data)` | `(data: unknown) => Promise<…>` | Async `safeParse`; the same runtime fallback as `parseAsync`. |
| `validate(data)` | `(data: unknown) => unknown` | Validation only, no output construction: returns the input reference when the schema accepts it, `null` when it does not. Typed `unknown` on purpose: the result is the caller's own object. Throws `$ZodAsyncError` when `async` is true, and when a plain function returned a `Promise` at parse time, as the other sync entries do; on a schema holding a transform that is not an async function (a `z.codec` decode included), a rejected input costs a second run through stock's parse, which is where that throw is decided (#79). On a schema holding a `z.lazy` anywhere (a recursive schema included) `validate` runs the CoW skeleton instead of the whole-tree validator, whose lazy check would read `.issues` off such a `Promise` and throw a `TypeError` (#90); the answer is the same, at the cost of the output construction a dirty child forces. |
