 ...
 *                       $ZodAsyncError like the other sync entries (#79); a tree holding a lazy has no whole-tree
 *                       validator and runs the skeleton, whose lazy island throws the same class (#90)
...
export interface Compiled<T extends z.ZodType> {
  ...
  /** Pure validation: on success returns the original input reference (typed `unknown`; unlike the zod3 line there is no DeepReadonly view), null on failure. Throws `$ZodAsyncError` when a plain function returned a `Promise`, as the other sync entries do; a tree holding a `lazy` runs the skeleton instead of the whole-tree validator for that (#90) */
