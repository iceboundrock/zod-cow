/**
 * zc-z4 — zod4 版 CoW 编译层公开 API（与 zod3 版 index.ts 同构）。
 *
 * 消费 stock zod4 的 schema（读取其 _zod.def 树），编译出零拷贝/按需拷贝的校验闭包。
 * 不 fork zod、不改 Zod API：schema 继续用于类型推断与普通 parse；
 * compile(schema) 额外提供：
 *
 *   .parse(data)     CoW 语义：成功返回值（可能 === 输入，结构共享）
 *   .validate(data)  同一运行时，返回类型标 DeepReadonly —— 把
 *                    “输出与输入共享”这一事实写进类型系统
...
export { PROBE4 } from "./probe-z4-flags.js";
