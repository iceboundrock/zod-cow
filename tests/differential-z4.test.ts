/**
 * 差分模糊测试（zod4 前端）—— 随机生成 schema + 数据，对比编译层与 stock zod4：
 *   1. 成败奇偶（success parity）
 *   2. 成功时输出值 deepStrictEqual
...
const REPRO = process.env.REPRO ?? null; // e.g. "112:80"
let total = 0;
let bothOk = 0;
let bothFail = 0;
let refShared = 0;
...
for (let seed = 1; seed <= SEEDS; seed++) {
  ...
  for (let i = 0; i < CASES_PER_SEED; i++) {
    ...
    if (REPRO && `${seed}:${i}` !== REPRO) continue; // RNG 已推进，仅跳过比对
    ...
    try {
      ...
      if (e instanceof ZcNotSupportedError) {
        continue; // 生成器碰到了编译面之外的特性（正常降级路径）
      }
    ...
    if (REPRO) {
      ...
      const el: any = (built.schema as any)._zod?.def?.element;
      if (el) {
        console.log("element optin:", el._zod?.optin, "optout:", el._zod?.optout);
...
console.log(`differential-z4: ${total} cases | success=${bothOk} fail=${bothFail}`);
